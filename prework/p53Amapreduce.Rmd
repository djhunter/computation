...
3. In Prework 4.2b we showed that $\mathit{CUR}_\text{DFA}$ is decidable by using a decider for $\mathit{EQ}_\text{DFA}$.  Following this argument, give an alternative proof that $\mathit{EQ}_\text{TM}$ is undecidable by showing that $\mathit{CUR}_\text{TM}$ reduces to $\mathit{EQ}_\text{TM}$.

4. [`r paste(fnames[7:9], sep = ',')`] Let $\Sigma = \{\mathtt{0},\mathtt{1}\}$. Show that a language $A$ is decidable if and only if $A \leq_\text{m} \{\mathtt{0}^\ast\mathtt{1}^\ast\}$. 
